package com.ucpaas.sms.util.security;
...
public class Des3Utils {
	 ...
	 * @return
	 */
	public static synchronized String encodeDes3(String str) {
		try {
			byte[] cipherStr = encode_cipher.doFinal(stringToByteArray(str));
			String temp = Base64.encodeBase64String(cipherStr);
	 ...
	 * @return
	 */
	public static synchronized String decodeDes3(String str) {
		String temp = str.replace("+", "%2B");
		temp = temp.replace("=", "%3D");
		temp = temp.replace("%25", "%");
		try {
			temp = URLDecoder.decode(temp, "utf-8");
			byte[] cipherStr = decode_cipher.doFinal(Base64.decodeBase64(temp));
